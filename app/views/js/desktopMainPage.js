...
java.addEventListener("click", javaCode);
php.addEventListener("click", phpCode);
...
function javaCode(){
    ...
    if(current_focus===schemaTextArea)
   {
        let sendJson={id:$user_id,data:schemaTextArea.value,language:"Java"};
        fetch("/app/generateCodeService/api/apiGC.php",{
        method: 'POST',
        header:'Content-type: application/json',
        body: JSON.stringify(sendJson)
     })
    ...
    .then(function(myJson) {
        ...
        if ( schemaResult.classList.contains("isError"))
         ...
         schemaResult.classList.add("isResult");
         sschemaText.innerHTML=myJson["response"].replace("\r\n","<br>");
    })    

}
else
 {
    let sendJson={id:$user_id,data:queryTextArea.value,language:"Java"};
    fetch("/app/generateCodeService/api/apiGC.php",{
    method: 'POST',
    header:'Content-type: application/json',
    body: JSON.stringify(sendJson)
...
.then(function(myJson) {
    ...
    if ( queryResult.classList.contains("isError"))
     ...
     queryResult.classList.add("isResult");
     qqueryText.innerHTML=myJson["response"].replace("\r\n","<br>");
})    
 }
...
function phpCode(){
    ...
    if(current_focus===schemaTextArea)
   {
        let sendJson={id:$user_id,data:schemaTextArea.value,language:"PHP"};
        fetch("/app/generateCodeService/api/apiGC.php",{
        method: 'POST',
        header:'Content-type: application/json',
        body: JSON.stringify(sendJson)
 ...
 {
    let sendJson={id:$user_id,data:queryTextArea.value,language:"PHP"};
    fetch("/app/generateCodeService/api/apiGC.php",{
    method: 'POST',
    header:'Content-type: application/json',
    body: JSON.stringify(sendJson)
